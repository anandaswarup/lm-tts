...
Setup:
    1. First, upload your dataset to Modal volume:
       modal run scripts/tokenize_libritts_r_modal.py::upload_dataset \
           --local-path /path/to/LibriTTS_R

    2. Deploy the tokenization function:
       modal deploy scripts/tokenize_libritts_r_modal.py
    3. Trigger the tokenization (runs in background):
       modal run scripts/tokenize_libritts_r_modal.py::trigger_tokenization \
